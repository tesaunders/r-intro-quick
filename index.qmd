...
Now that we know it works, let's use it to create plots for all 36 species. We're going to use the `map()` function from the {purrr} package to do this. This function takes a list or vector (in our case unique values from the species_id column), and a function to apply to all values (in our case, the plot function we just created):
